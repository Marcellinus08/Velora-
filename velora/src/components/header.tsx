...
// Dynamic import (no SSR) dari komponen ConnectWalletButton
const ConnectWalletButton = dynamic(
  () => import("@/components/connect-wallet-button").then((m) => m.ConnectWalletButton),
  { ssr: false }
);
...
export default function Header() {
  ...
  // --- wallet info untuk custom menu (pakai hooks ringan) ---
  const { address } = useAccount();
  const { logout } = useLoginWithAbstract();

  // Simpel notifications (bisa dikembangkan)
  const [notifications] = useState<Notif[]>([]);
  const unreadCount = notifications.filter((n) => n.unread).length;

  // --- Build custom dropdown items for ConnectWalletButton ---
  const customDropdownItems = [
    // Profile area (nama + address)
    ...
    // Disconnect (memanggil logout dari agw)
    <DropdownMenuItem
      key="disconnect"
      onClick={() => {
        logout();
  ...
  return (
    ...
    >
        ...
        <div ref={containerRef} className="relative w-full max-w-[720px]">
          <form onSubmit={onSubmit} className="flex w-full items-center">
            ...
            <button
              type="button"
              onClick={() => {
                // voice placeholder: keep original voice() logic if you want
              }}
              className="ml-3 flex h-10 w-10 items-center justify-center rounded-full bg-neutral-800 text-neutral-200 hover:bg-neutral-700"
              aria-label="Pencarian suara"
              title="Pencarian suara"
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        {/* Sekarang: satu tombol yang memuat wallet + profile menu */}
        <ConnectWalletButton className="min-w-28 px-4" customDropdownItems={customDropdownItems} />
      </div>
    </header>
  );
}
